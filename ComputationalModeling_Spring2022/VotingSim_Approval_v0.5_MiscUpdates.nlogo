turtles-own [
  ...
  total-votes          ; The number of votes that a party held at the end of the last tick
]

patches-own [
  preferred-party     ; The party currently preferred most by the voter/patch
]

globals [
  voters           ; The patches that will act as voters, either a vertical line or all patches
]
...
to setup-voters   ; party the agentset of patches that are going to be our voters in a global variable for easy reference.
  set voters patches
end

to setup-parties   ; We choose as many random colors as the number of parties we want to create.
  ; NOTE: In reality, there are colors associated with specific party ideologies (ex. yellow = liberal, blue = conservative); as these are not pertinent to the model they are
  ; not maintained in the party list here
  foreach n-of number-of-parties base-colors [ c ->  ; ...and we create a party of each of these colors on random voter patches, here representing ideology
    ask one-of voters [
      sprout 1 [
...
to go
  ; We accumulate platform changes as lists of tasks to be run later in order to simulate simultaneous decision making on the part of the parties
  if ticks >= 200 [stop]
  let platform-changes  [ new-platform-task ] of turtles

  foreach platform-changes run
    recalculate-area
  tick
end

to recalculate-area ; Have each consumer (patch) indicate its preference by ; taking on the color of the party it chooses
  ask voters [
    set preferred-party choose-party ; identifies party closes to patch; even in approval voting, voters will have a first choice!
  ]
  ask turtles [
    set total-votes count voters in-radius approval-range ; APPROVAL VOTING: now get votes from ANY patch within the global approval range used by the electorate
  ]
  ask patches [set pcolor black]  ; each cycle the visualization reverts to null
  ask turtles
  [ ask patches in-radius approval-range ; before indicating the preferred candidate by the patches' color.
    ; NOTE: Unlike other simulations, if a patch has no candidate in range, they will simply cast a ballot showing approval for no candidates; a 'None of the Above' option
      [ set pcolor ([ color ] of preferred-party - 2 )] ] ;; each candidate has a 'halo' of voters surrounding it
end

;;;;;;;;;;;;;;;;;;;;;;;;;
;;; candidate (turtle) procedures ;;;
;;;;;;;;;;;;;;;;;;;;;;;;;

; Have the party consider the benefits of making a platform adjustment- represented by a unit step- in each of the four cardinal directions on the ideological spectrum
; and report a task that will allow the chosen platform change to be enacted later
to-report new-platform-task
  ...
  if total-votes > 0 [
    ; Only consider the status quo if we already have a voter base, but if we consider it,
    ; put it at the front of the list so it is favored in case of ties in sort-by, representing ideological inertia in the party
    set possible-moves fput patch-here possible-moves
  ]

  ; pair the potential moves with their voter shares, and sort these pairs by voter share
  let moves-with-voter-shares
    sort-by [ [a b] -> last a > last b ]
    map [ move -> list move (voter-share-if-move-to move) ] possible-moves

  ; report the first item of the first pair, i.e., the move with the best voter share
  let chosen-platform first first moves-with-voter-shares

  let party self ; put self in a local variable so that it can be "captured" by the task
  report [ ->
    ask party [
      pen-down
      move-to chosen-platform
      pen-up
    ]
  ]
end

; report the voter share area the party would have if it moved to destination
to-report voter-share-if-move-to [ destination ] ; turtle procedure
  let current-position patch-here
  move-to destination
  let voter-share-at-destination potential-voter-share
  move-to current-position
  report voter-share-at-destination
end

to-report potential-voter-share
  report count voters in-radius approval-range
end



;;;;;;;;;;;;;;;;;;;;;;;
;;; patch (voter) procedure ;;;
;;;;;;;;;;;;;;;;;;;;;;;

; report the party with the best deal, defined as the smallest sum of price and distance
to-report choose-party
  report min-one-of turtles [ (distance myself) ]
end

to-report winner-platform-xcor
  report [xcor] of min-one-of turtles [ total-votes ]
end

to-report winner-platform-ycor
  report [ycor] of min-one-of turtles [ total-votes ]
end

to-report party-paths-turtle-0
 ifelse number-of-parties >= 1
  [show [list xcor ycor] of turtle 0]
  [print "party not present"]
end

to-report party-paths-turtle-1
 ifelse number-of-parties >= 2
  [show [list xcor ycor] of turtle 1]
  [print "party not present"]
end

to-report party-paths-turtle-2
 ifelse number-of-parties >= 3
  [show [list xcor ycor] of turtle 2]
  [print "party not present"]
end

to-report party-paths-turtle-3
 ifelse number-of-parties >= 4
  [show [list xcor ycor] of turtle 3]
  [print "party not present"]
end

to-report party-paths-turtle-4
 ifelse number-of-parties >= 5
  [show [list xcor ycor] of turtle 4]
  [print "party not present"]
end

to-report party-paths-turtle-5
 ifelse number-of-parties >= 6
  [show [list xcor ycor] of turtle 5]
  [print "party not present"]
end

to-report party-paths-turtle-6
 ifelse number-of-parties >= 7
  [show [list xcor ycor] of turtle 6]
  [print "party not present"]
end

to-report party-paths-turtle-7
 ifelse number-of-parties >= 8
  [show [list xcor ycor] of turtle 7]
  [print "party not present"]
end

to-report party-paths-turtle-8
 ifelse number-of-parties >= 9
  [show [list xcor ycor] of turtle 8]
  [print "party not present"]
end

to-report party-paths-turtle-9
 ifelse number-of-parties >= 10
  [show [list xcor ycor] of turtle 9]
  [print "party not present"]
end

to-report party-paths-turtle-10
 ifelse number-of-parties >= 11
  [show [list xcor ycor] of turtle 10]
  [print "party not present"]
end

to-report party-paths-turtle-11
 ifelse number-of-parties >= 12
  [show [list xcor ycor] of turtle 11]
  [print "party not present"]
end

to-report party-paths-turtle-12
 ifelse number-of-parties >= 13
  [show [list xcor ycor] of turtle 12]
  [print "party not present"]
end

to-report party-paths-turtle-13
 ifelse number-of-parties >= 14
  [show [list xcor ycor] of turtle 13]
  [print "party not present"]
end
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot winner-platform-xcor"
...
"" ""
PENS
"default" 1.0 0 -16777216 true "" "plot winner-platform-ycor"
...
<experiments>
  <experiment name="experiment" repetitions="1" runMetricsEveryStep="true">
    ...
    <enumeratedValueSet variable="number-of-parties">
      <value value="11"/>
    </enumeratedValueSet>
    <enumeratedValueSet variable="approval-range">
      <value value="13"/>
    </enumeratedValueSet>
  ...
  <experiment name="test-run-approval-voting" repetitions="1" runMetricsEveryStep="true">
    <setup>setup</setup>
    <go>go</go>
    <metric>winner-platform-xcor</metric>
    <metric>winner-platform-ycor</metric>
    <metric>party-paths-turtle-0</metric>
    <metric>party-paths-turtle-1</metric>
    <metric>party-paths-turtle-2</metric>
    <metric>party-paths-turtle-3</metric>
    <metric>party-paths-turtle-4</metric>
    <metric>party-paths-turtle-5</metric>
    <metric>party-paths-turtle-6</metric>
    <steppedValueSet variable="number-of-parties" first="2" step="1" last="7"/>
    <steppedValueSet variable="approval-range" first="1" step="6" last="33"/>
  </experiment>
</experiments>
@#$#@#$#@
@#$#@#$#@
