...
patches-own [
  ...
  second-choice-party     ; The party currently preferred most by the voter/patch
]
...
to setup-voters
  ; party the agentset of patches that are going to be our
...
to setup-parties
  ; We choose as many random colors as the number of partys we want to create
  foreach n-of number-of-parties base-colors [ c ->
    ; ...and we create a party of each of these colors on random consumer patches
    ask one-of voters [
      sprout 1 [
        set color c ; use the color from the list that we are looping through
        set shape "Circle (2)"
        set size 2
        ;set price 10; not using price
        set pen-size 5
      ]
    ]
  ]
...
to go
  ; We accumulate platform and price changes as lists of tasks to be run later
  ; in order to simulate simultaneous decision making on the part of the partys
  if ticks >= 200 [stop]
  let platform-changes ifelse-value rules = "pricing-only"
    [ (list) ] ; if we are doing "pricing-only", the list of moves is empty
    [ [ new-platform-task ] of turtles ]
...
to recalculate-area
  ...
  ; taking on the color of the party it chooses
  ask voters [
    set preferred-party choose-party
    set second-choice-party choose-party-second
    set pcolor ([ color ] of preferred-party + 2)
  ]
  ask turtles [
    set total-votes count voters in-radius 20 ; APPROVAL VOTING: now get votes from ANY patch within 15 'patches'
  ]
end
...
; Have the party consider the benefits of taking a unit step in each of the four cardinal directions
...
; report the market share area the party would have if it moved to destination
...
to-report potential-voter-share
  report count voters in-radius 20
end

; Have the party consider the revenue from hypothetically increasing or decreasing its price by one unit
...
to parties-collapse
  ask turtles with [total-votes <= .05 * (count patches) ] [ die]
end
...
to-report choose-party-second
; let parties-excluding-favorite turtles with [turtles != choose-party]
;  let parties-excluding-favorite turtles with [not member? self choose-party]
  let parties-excluding-favorite other (turtle-set choose-party)
  report min-one-of turtles [ (distance myself) ]
end
